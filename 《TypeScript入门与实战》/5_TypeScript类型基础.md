...
当启用了“--strictNullChecks”编译选项时，只允许将undefined值赋值给void类型。如果没启用，将运行undefined和null.
...
unknown类型是比any类型更安全的顶端类型，因为unknows类型只允许负值给any类型和unknown类型。
...
场景一：只有在函数更笨无法返回一个值的时候，函数的返回值类型才是never类型。

若函数中的代码不是直接抛出异常二是间接地抛出异常，那么函数的返回值类型也是never类型。
...
Tuple，元组类型hi数组类型的子类型。元组是长度规定的数组，并且元组中每个元素都有确定的类型。
...
Object类型是特殊对象“Object.prototype”的烈性，该类型的主要作用是描述JavaScript中几乎素有对象都共享（通过原型继承）的属性和方法。
