...
TypeScript编译器主要能够识别一下几类代码结构并进行类型细化：
...
实际上，类型守卫早已融入我们的代码当中，我们通常不需要为类型守卫座额外的编码工作，他们已经默默地发挥作用。

#### 6.11.1.1 类型守卫

typeof类型守卫能够根据typeof表达式的值取细化typeof操作数的类型。

typeof运算符返回值也是一个有限集合：

| 操作数的类型  | typeof返回值 | 操作数的类型 | typeof返回值 |
| ------------- | ------------ | ------------ | ------------ |
| Undefined类型 | "undefined"  | Symbol类型   | “symbol”     |
| Null类型      | "object"     | BigInt类型   | "bigint"     |
| Boolean类型   | "boolean"    | 函数类型     | "function"   |
| Number类型    | "number"     | 对象类型     | "object"     |
| String 类型   | "string"     |              |              |

typeof类型守卫在细化运算结果为“object”的类型时，会包含null类型。

**带调用签名的对象类型是函数类型。**接口表示一种对象类型。

#### 6.11.1.2 instanceof类型守卫
...
| Null      | false                                              |
| Boolean   | 结果不变                                           |
| Number    | 若数字位+0、-0或NaN，结果为false；否则，结果为true |
| String    | 若字符串为空字符串，结果为false；否则结果为true    |
| Symbol    | true                                               |
| BigInt    | 若值为0n，结果为false，否则，结果为true            |
| Object    | true                                               |

需要注意的是，如果在对象属性上使用了逻辑与、或、非类型守卫，而后一对该独享属性进行了赋值操作，那么类型守卫将失效，不会进行类型细化。

#### 6.11.1.5 等式类型守卫

===  !== == !=

若判定一个值严格等于undefined值，则将该值细化为undefined类型，而不是细化为联合类型“undefined | null”

但如果等式类型守卫中使用的是非严格相等运算“==”和“!=”，那么类型细化时会将undefined类型和null类型视为相同的空类型。

除了undefined值和null值之外，等式类型守卫还支持一下种类的字面量：
...
等式类型守卫也支持将两个类型参数或变量进行等式比较，并同时细化两个操作数的类型。

在switch语句中，每一个case分支语句都相当于等式类型守卫。

#### 6.11.1.6 自定义类型守卫函数

类型守卫函数是指在函数返回值类型中使用类型谓词的函数。
...
编译器能够利用该性质并结合switch语句来对可辨识联合类型进行完整性检查。

### 6.11.3 赋值语句分析

根据等号右侧操作数的类型去细化左侧操作数的类型。

如果在变量或参数声明中包含了类型注解，那么在进行类型细化时同样会参考变量声明的类型。

### 6.11.4 基于控制流的类型分析

TypeScript编译器能够分析程序代码中所有可能的执行路径，从而得到在代码中某一特定位置上的变量类型和参数类型等。
...
或 function assert(x: unknown): asserts x {}
```

若一个函数带有asserts类型判定，那么该函数就是一个断言函数。

#### 6.11.5.1 asserts x is T

开发者需要按照约定的断言函数语义去实现断言函数。
...
在定义段晗函数时，我们需要将函数的返回值类型声明为asserts类型判定。编译器将asserts类型判定视为void类型，这意味着断言函数的返回值类型是void。
...
基于控制流的类型分析能够利用以上的事实调用断言函数之后的代码进行类型细化。
